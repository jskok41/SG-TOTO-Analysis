...
export default function Home() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        const response = await fetch('/api/toto-data');
        const result = await response.json();
        
        if (result.data) {
          setData(result.data);
          const analyzerInstance = new TotoAnalyzer(result.data);
          ...
          <TabsContent value="statistics" className="space-y-6">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <Card>
                ...
                <CardContent>
                  <div className="space-y-3">
                    {summary?.mostFrequentNumbers.slice(0, 10).map((item, index) => (
              ...
              <Card>
                ...
                <CardContent>
                  <div className="space-y-3">
                    {summary?.leastFrequentNumbers.slice(0, 10).map((item, index) => (
                      <div key={item.number} className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          <Badge variant="outline">{item.number}</Badge>
                          <span className="text-sm text-muted-foreground">
